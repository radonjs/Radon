...
// my actual shit, will delete everything above this point ^

// import all state nodes
...
/*export*/ combineNodes = (...args) => {
  ...
  args.forEach(node => {
    if (node.getParent() === null) {
      hashTable.root = node;
    } else if (!hashTable[node.getParent()]) {
      hashTable[node.getParent()] = [node];
    } else {
      hashTable[node.getParent()].push(node);
    }
  })
  console.log(hashTable);

  store[hashTable.root.getName()] = {
    value: hashTable.root.getState(),
    parent: hashTable.root.getParent()
  }
